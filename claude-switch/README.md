...
Multi-account switcher for Claude Code on macOS — unlimited subscriptions, stored in the Keychain.
...
Auto-detected from token, or chosen interactively during setup:
...
./claude-switch/install.sh --short
```
...
```bash
chmod +x claude-switch/claude-switch
./claude-switch/install.sh
